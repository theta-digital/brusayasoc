...
if ( ! function_exists( 'justitia_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'justitia_hovers_theme_setup3', 3 );
	function justitia_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		justitia_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'justitia' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'justitia' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'justitia' ),
								),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'justitia' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'justitia' ) ),
					'std'      => 'icon',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'justitia' ),
					),
					'options'  => justitia_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'justitia_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'justitia_hovers_theme_setup9', 9 );
	function justitia_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'justitia_hovers_frontend_scripts', 1010 );
		add_filter( 'justitia_filter_localize_script', 'justitia_hovers_localize_script' );
		add_filter( 'justitia_filter_merge_scripts', 'justitia_hovers_merge_scripts' );
		add_filter( 'justitia_filter_merge_styles', 'justitia_hovers_merge_styles' );
		add_filter( 'justitia_filter_merge_styles_responsive', 'justitia_hovers_merge_styles_responsive' );
		add_filter( 'justitia_filter_get_css', 'justitia_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'justitia_hovers_frontend_scripts' ) ) {
		function justitia_hovers_frontend_scripts() {
		if ( justitia_is_on( justitia_get_theme_option( 'debug_mode' ) ) ) {
			$justitia_url = justitia_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $justitia_url ) {
				wp_enqueue_script( 'justitia-hovers', $justitia_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'justitia_hovers_merge_styles' ) ) {
		function justitia_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'justitia_hovers_merge_styles_responsive' ) ) {
		function justitia_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'justitia_hovers_localize_script' ) ) {
		function justitia_hovers_localize_script( $arr ) {
		$arr['button_hover'] = justitia_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'justitia_hovers_merge_scripts' ) ) {
		function justitia_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'justitia_hovers_add_icons' ) ) {
	function justitia_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php justitia_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php justitia_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php justitia_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php justitia_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php justitia_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'justitia_hovers_get_css' ) ) {
		function justitia_hovers_get_css( $css, $args ) {
