...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'justitia_trx_addons_theme_specific_setup1' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'justitia_trx_addons_theme_specific_components' );
	function justitia_trx_addons_theme_specific_components( $enable = true ) {
		return false;
	}
}

if ( ! function_exists( 'justitia_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'justitia_trx_addons_theme_specific_setup1', 1 );
	function justitia_trx_addons_theme_specific_setup1() {
		if ( justitia_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'justitia_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'justitia_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'justitia_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_cpt_list', 'justitia_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'justitia_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'justitia_trx_addons_widgets_list' );
		}
	}
}

// CV
if ( ! function_exists( 'justitia_trx_addons_cv_enable' ) ) {
		function justitia_trx_addons_cv_enable( $enable = false ) {
		// To do: return false if theme not use CV functionality
		return JUSTITIA_THEME_FREE
					? false     // Free version
					: true;     // Pro version
	}
}
...
// Demo mode
if ( ! function_exists( 'justitia_trx_addons_demo_enable' ) ) {
		function justitia_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'justitia_trx_addons_edd_themes_market_enable' ) ) {
		function justitia_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'justitia_trx_addons_api_list' ) ) {
		function justitia_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'justitia_trx_addons_cpt_list' ) ) {
		function justitia_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'justitia_trx_addons_sc_list' ) ) {
		function justitia_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'justitia_trx_addons_widgets_list' ) ) {
		function justitia_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'justitia_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'justitia_trx_addons_menu_cache' );
	function justitia_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'justitia_trx_addons_localize_script' ) ) {
	...
	function justitia_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = justitia_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'justitia_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'justitia_trx_addons_sc_type', 10, 2 );
	function justitia_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = justitia_array_merge( $list, justitia_get_list_blog_styles( false, 'sc' ) );
		}
        if($sc == 'trx_sc_icons') {
            $list['simple'] = 'Simple';
        }
        ...
        if($sc == 'trx_sc_services') {
            $list['numbered-image'] = 'Numbered image';
        }
...
if ( ! function_exists( 'justitia_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'justitia_trx_addons_sc_prepare_atts', 10, 2 );
	function justitia_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = justitia_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = justitia_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'justitia_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'justitia_trx_addons_sc_atts', 10, 2 );
	function justitia_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'justitia_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'justitia_trx_addons_sc_map', 10, 2 );
	function justitia_trx_addons_sc_map( $params, $sc ) {
		...
		if ( in_array(
			...
			$params['params'][] = array(
				'param_name'  => 'scheme',
				'heading'     => esc_html__( 'Color scheme', 'justitia' ),
				'description' => wp_kses_data( __( 'Select color scheme to decorate this block', 'justitia' ) ),
				'group'       => esc_html__( 'Colors', 'justitia' ),
				'admin_label' => true,
				'value'       => array_flip( justitia_get_list_schemes( true ) ),
				'type'        => 'dropdown',
			);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'justitia' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'justitia' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( justitia_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'justitia' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'justitia' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( justitia_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'justitia_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'justitia_trx_addons_sow_map', 10, 2 );
	function justitia_trx_addons_sow_map( $params, $sc ) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label'       => esc_html__( 'Color style', 'justitia' ),
				'description' => wp_kses_data( __( 'Select color style to decorate this block', 'justitia' ) ),
				'options'     => justitia_get_list_sc_color_styles(),
				'default'     => 'default',
				'type'        => 'select',
			),
		);
		if ( in_array( $sc, array( 'trx_sc_button' ) ) ) {
			justitia_array_insert_after( $params, 'size', $param );
		} elseif ( in_array(
			$sc, array(
				'trx_sc_action',
				'trx_sc_blogger',
			...
			justitia_array_insert_after( $params, 'title_align', $param );
		}
		return $params;
	}
}
...
if ( ! function_exists( 'justitia_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'justitia_trx_addons_sc_output', 10, 4 );
	function justitia_trx_addons_sc_output( $output, $sc, $atts, $content ) {

		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			if ( ! empty( $atts['scheme'] ) && ! justitia_is_inherit( $atts['scheme'] ) ) {
				$output = str_replace( 'class="sc_action ', 'class="sc_action scheme_' . esc_attr( $atts['scheme'] ) . ' ', $output );
			}
			if ( ! empty( $atts['color_style'] ) && ! justitia_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'justitia_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'justitia_trx_addons_sc_item_title_tag' );
	function justitia_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'justitia_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'justitia_trx_addons_sc_item_button_args', 10, 3 );
	function justitia_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'justitia_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'justitia_trx_addons_slider_title', 10, 2 );
	function justitia_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'justitia_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'justitia_trx_addons_sc_googlemap_styles' );
	function justitia_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'justitia' );
		return $list;
	}
}
...
if ( ! function_exists( 'justitia_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'justitia_trx_addons_tiny_mce_style_formats' );
	function justitia_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'justitia' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'justitia' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'justitia_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'justitia_filter_allow_override_header_image', 'justitia_trx_addons_allow_override_header_image' );
	function justitia_trx_addons_allow_override_header_image( $allow ) {
		return justitia_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( ! function_exists( 'justitia_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'justitia_trx_addons_thumb_size', 10, 2 );
	function justitia_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
...
if ( ! function_exists( 'justitia_trx_addons_override_options_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'justitia_trx_addons_override_options_fields', 10, 2 );
	function justitia_trx_addons_override_options_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'justitia' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'justitia' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
...
if ( ! function_exists( 'justitia_filter_get_list_menu_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_menu_hover', 'justitia_filter_get_list_menu_hover' );
    function justitia_filter_get_list_menu_hover( $list ) {
        unset( $list['fade_box'] );
...
if ( ! function_exists( 'justitia_filter_get_list_input_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_input_hover', 'justitia_filter_get_list_input_hover' );
    function justitia_filter_get_list_input_hover( $list ) {
        unset( $list['accent'] );
        unset( $list['path'] );
        unset( $list['jump'] );
