...
if ( ! empty( $justitia_template_args['slider'] ) ) {
	...
	echo ( 'classic' == $justitia_blog_style[0] ? 'column' : 'masonry_item masonry_item' ) . '-1_' . esc_attr( $justitia_columns );
}
?>
	...
	<?php
		post_class(
			'post_item post_format_' . esc_attr( $justitia_post_format )
					. ' post_layout_classic post_layout_classic_' . esc_attr( $justitia_columns )
	...
	justitia_show_post_featured(
		array(
			'thumb_size' => justitia_get_thumb_size(
				'classic' == $justitia_blog_style[0]
						? ( strpos( justitia_get_theme_option( 'body_style' ), 'full' ) !== false
								? ( $justitia_columns > 2 ? 'big' : 'huge' )
								: ( $justitia_columns > 2
									? ( $justitia_expanded ? 'big' : 'big' )
									: ( $justitia_expanded ? 'big' : 'big' )
									)
							)
						: ( strpos( justitia_get_theme_option( 'body_style' ), 'full' ) !== false
								? ( $justitia_columns > 2 ? 'masonry-big' : 'full' )
								: ( $justitia_columns <= 2 && $justitia_expanded ? 'masonry-big' : 'masonry' )
							)
			),
