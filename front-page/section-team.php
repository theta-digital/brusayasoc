<div class="front_page_section front_page_section_team<?php
	$justitia_scheme = justitia_get_theme_option( 'front_page_team_scheme' );
	if ( ! justitia_is_inherit( $justitia_scheme ) ) {
		echo ' scheme_' . esc_attr( $justitia_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( justitia_get_theme_option( 'front_page_team_paddings' ) );
?>"
		<?php
		$justitia_css      = '';
		$justitia_bg_image = justitia_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$justitia_anchor_icon = justitia_get_theme_option( 'front_page_team_anchor_icon' );
	$justitia_anchor_text = justitia_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( justitia_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$justitia_css      = '';
			$justitia_bg_mask  = justitia_get_theme_option( 'front_page_team_bg_mask' );
			$justitia_bg_color_type = justitia_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $justitia_bg_color_type ) {
				$justitia_bg_color = justitia_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$justitia_caption = justitia_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $justitia_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $justitia_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $justitia_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$justitia_description = justitia_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $justitia_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $justitia_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $justitia_description ), 'justitia_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
				...
				if ( ! justitia_exists_trx_addons() ) {
					...
					justitia_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
				}
			}
			?>
			</div>
