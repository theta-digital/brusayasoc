	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="justitia_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.justitia_post_editor' );
					if ( link.length > 0 ) {
						justitia_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.justitia_post_editor', function(e) {
					justitia_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('justitia_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function justitia_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
